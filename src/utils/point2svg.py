...
def patch_fill(img, div_num):
    ...
    ImageOutput = Image.fromarray(img)
    ImageOutput.save("./output/total.png")
...
class Point2svg(object):
    def __init__(
        ...
        for patch_idx in range(self.div_num * self.div_num):
            self.all_patch_actions.append([])
            self.patch_bgcolor_list.append([])
        self.START_TXT = """<svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="{}" width="{}" viewBox="0 0 {} {}"><g transform="scale({}, {})">\n<rect x="0" y="0" width="{}" height="{}" fill="white"/>\n{}\n""".format(
            self.output_width,
            self.output_width * self.scaley,
            self.output_width,
            self.output_width * self.scaley,
            self.scalex,
            self.scaley,
        ...
        if self.use_patch_fill:
            self.patch_fill_svg(
                img=img, div_num=self.div_num, svg_size=self.output_width
            )
        self.element_list = []
        self.END_TXT = """</g></svg>"""
        self.svg_txt_total = ""
    ...
    def patch_fill_svg(self, img, div_num, svg_size):
        ...
        self.fill_list = []
        for patch_idx in range(self.div_num * self.div_num):
            self.done_fill_list.append([])
            self.not_done_fill_list.append([])
            self.fill_list.append([])

        (h, w) = img.shape
        max_size = max(h, w)
        svg_patch_size = float(svg_size) / div_num
        ...
        img = vu.gray_div(img)
        svg_total = []
        patch_total = self.div_num * self.div_num
        for patch_num in range(patch_total):
            patch_coord = self.patchnum2coord(patch_num=patch_num)
            ...
            if gray != 255 and p >= 0.7 and p != 1:
                svg_txt = (
                    """<rect x="{}" y="{}" width="{}" height="{}" fill="{}"/>""".format(
                        xr, yr, svg_patch_size, svg_patch_size, color
                    )
                )
    ...
    def add_action_div(
        self, new_action, stroke_dim=8, color_dim=1
    ):  # torch.Size([256, 65])
        new_action = new_action.detach().view(
            self.div_num * self.div_num, -1, stroke_dim + color_dim
        )
    ...
    def get_svg_txt_by_path(self, action, width, patch_coord):
        ...
        t_point = [i * rate * tmp for i in t_range]

        list_point1 = []
        list_point2 = []
        for t in t_point:
            points1, points2 = self.get_get_tangent_point_by_t(p=p, t=t)
        ...
        # 根据点拟合曲线
        fit_cubic = fit_curves.fit_curve(points=edge_point1_list, max_error=10)
        fit_cubic2 = fit_curves.fit_curve(points=edge_point2_list, max_error=10)

        fit = np.round(fit_cubic, 2)
        fit2 = np.round(fit_cubic2, 2)

        # 开始和结束的圆
        start_cycle = self.get_cyclev2(x0, y0, z0, color)
        rate_end = 10000
        tmp_end = 1.0 / rate_end
        ...
        endr = (1 - t_end) * z0 + t_end * z2
        end_cycle = self.get_cyclev2(
            np.around(endx, 2), np.around(endy, 2), np.around(endr, 2), color
        )

        # 边缘
        svg_edge_first = self.curvepoint2svg(fit=fit)
        svg_edge_second = self.curvepoint2svg(fit=fit2)
        svg_edge = self.get_edgev2(
            np.round(list_point1[0], 2),
            np.round(list_point2[0], 2),
            svg_edge_first,
            svg_edge_second,
            color=color,
        )
        ...
        for patch_num in range(len(self.all_patch_actions)):
            ...
            if self.patch_done == "not_done":
                ...
                for action_single in one_patch_action:
                    self.get_svg_txt_by_path(
                        action=action_single, width=self.width, patch_coord=patch_coord
                    )
                config = [
                    self.patch_svg_list.copy(),
                    self.width,
                    self.gt[patch_num],
                    patch_coord,
                    self.coord_bais,
