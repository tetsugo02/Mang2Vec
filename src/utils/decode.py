...
from src.processing import mythread
...
class Decode_np(object):
    def __init__(self, div_num, use_PM=True):
        ...
        self.patch_bgcolor_list = np.load(
            self.np_path + "patch_bgcolor_list.npy", allow_pickle=True
        )
        ...
        if self.use_PM:
            for patch_num in range(len(self.all_patch_actions)):
                ...
                if self.patch_done == "not_done":
                    m = mythread.myThread(
                        config, self.patch_bgcolor_list, self.fill_list
                    )
                    svg_txt_one_patch = m.run()
                    self.svg_txt_total = "{}\n{}".format(
                        self.svg_txt_total, svg_txt_one_patch
                    )
