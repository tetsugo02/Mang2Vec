...
def cal_dis(svg_text, gt, svg_s, svg_e, patch_num, width=128):
    ...
    dis = (
        np.abs(current_png.astype("float32") - gt.astype("float32")).mean(0).mean(0)
        / width
        / width
    )
...
def simplify_patch_action(
    svg_text, width, color, gt_patch, patch_coord, coord_bais, div_num, patch_num
):
    [i, j] = patch_coord
    [cx, cy] = coord_bais
    baisx = cx[i][j]
    ...
    while flag != "done":
        num = num + 1
        last_dis, svg_text, flag, current_png = delete_svg(
            last_dis, svg_text, gt, svg_s, svg_e, patch_num
        )
        # print(num, last_dis)
...
def simplify_patch_actionv_no_PM(
    svg_text, width, color, gt_patch, patch_coord, coord_bais, div_num, patch_num
):
    [i, j] = patch_coord
    [cx, cy] = coord_bais
...
class myThread(object):
    ...
    def __init__(self, config, patch_bgcolor_list, fill_list, use_PM=True):
        ...
        self.svg_txt_one_patch_s = (
            """<g style="clip-path: url(#clipPath{}); ">""".format(self.patch_num)
        )
    ...
    def run_no_PM(self):
        self.svg_text = simplify_patch_actionv_no_PM(
            svg_text=self.svg_text,
            width=self.width,
            color=self.color,
            gt_patch=self.gt_patch,
